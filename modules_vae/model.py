...
class MultiModalVAE(torch.nn.Module):
    def __init__(self,
        ...
        for input_type, input_dim, layer_dim in zip(input_types, input_dims, layer_dims):
            setattr(self, f'encoder_{input_type}', buildNetwork([input_dim] + layer_dim, activation=torch.nn.Tanh()))
            setattr(self, f'decoder_{input_type}', buildNetwork(layer_dim[::-1] + [input_dim], activation=torch.nn.Tanh()))
        ...
        self.joint_encoder_mu = buildNetwork([self.joint_encoders_dim,z_dim*2,z_dim],activation=torch.nn.Tanh())
        self.joint_encoder_log_sigma = buildNetwork([self.joint_encoders_dim,z_dim*2,z_dim],activation=torch.nn.Tanh())
        self.joint_decoder = buildNetwork([z_dim, self.joint_encoders_dim], activation=torch.nn.Tanh())
